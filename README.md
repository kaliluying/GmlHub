...
- **服务状态监控** - 以“可访问 / 不可达 / 待检查”表示网络可达性，不冒充业务健康检查
- **置顶 & 最近** - 常用应用快速访问
- **项目深链接** - 使用 `#/app/:id` 直接打开项目内容页
- **PWA** - 支持安装到桌面，并提供基础离线壳
...
│   └── ...
├── stores/
│   └── desktop.js       # 桌面状态管理与窗口边界
├── apps/
│   └── registry.js       # 可扩展应用插件注册表
├── config/
│   ├── apps.js           # 应用 manifest
│   └── settings.js       # 默认设置与校验
├── services/
│   ├── deepLinks.js      # 项目深链接
│   └── persistence.js    # 版本化本地持久化与旧键迁移
├── App.vue              # 根组件
└── main.js              # 入口文件
```
